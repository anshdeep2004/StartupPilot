...
router.post('/project/:projectId', authenticate, async (req, res, next) => {
  try {
    ...
    // enforce assignment rules: only ADMIN can assign arbitrary users
    let finalAssignee = null;
    if (req.user.role === 'ADMIN') {
      finalAssignee = assigneeId || null;
    } else {
      // non-admins: either assign to self or leave unassigned
      finalAssignee = req.user.id;
    }
    const created = await prisma.task.create({ data: { projectId: req.params.projectId, title, description, status: status || 'todo', assigneeId: finalAssignee, dueDate: dueDate ? new Date(dueDate) : null, orderIndex } });
...
router.put('/:id', authenticate, async (req, res, next) => {
  try {
    ...
    // non-admins are not allowed to change assignee to other users
    let finalData = { title, description, status, orderIndex };
    if (req.user.role === 'ADMIN') {
      finalData.assigneeId = assigneeId;
    }
    const updated = await prisma.task.update({ where: { id: req.params.id }, data: finalData });
    res.json(updated);
  } catch (err) { next(err); }
});
